...
export function initCFCApp() {
  ...
  on($('referCode'), 'click', function () { say('Referral code AARTHI200 copied'); });
  ...
  /* ---- category tiles add nothing to the cart, they navigate ---- */
  Array.prototype.forEach.call(document.querySelectorAll('.cat'), function (c) {
    on(c, 'click', function () {
      var name = c.querySelector('h3');
      say((name ? name.textContent : 'Category') + ' — service list not built yet');
    });
  });
